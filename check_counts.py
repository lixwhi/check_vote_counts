		...
		if (previous_bn != current_bn):
			...
			for spell in spells:
				if spell['active'] == True:
					spell['count'] = int(web3.toHex(bytes(web3.eth.getStorageAt(CHIEF, web3.toInt(hexstr=spell['address']))))[2:66].lstrip('0'), 16)

			# print the counts
			for spell in spells:
				if spell['active'] == True:
					print('{description} count: {count}'.format(description=spell['description'], count=str(spell['count']/ETH_SCALE)) )
